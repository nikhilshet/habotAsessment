...
function DiveIn() {
    ...
    // transition-all duration-200 hover:bg-[#E7760D] hover:text-white

    const locationElements = locations.map((ele , index)=>{
        return(
            <div key={index}  className='w-64 h-16 rounded-md bg-white border-1 flex justify-center items-center transition-all duration-200 border-[#E7760D] text-black hover:bg-[#E7760D] hover:text-white'>
  ...
  return (
    <section className='flex flex-col px-4 py-24 justify-center items-center space-y-16 xl:flex-row xl:justify-between xl:px-32'>
        <div className=' w-full flex flex-col justify-center space-y-6 items-center xl:w-1/3 '>
            ...
            <button className='bg-green-700 text-white text-md font-semibold py-3 px-3 flex items-center rounded-md mb-4 transition-all duration-200 hover:bg-white hover:text-green-600 hover:border-1 hover:border-green-600'>Sign up Today !<img className='ml-4'  src={arrow}></img></button>
        </div>
